...
| `AiSuggestionsPanel` | AI 学习建议（负载、进度、概览） |

| `DailyRefinementPanel` | 每日计划细化（Markdown + 完成标准清单） |

| `aiSuggestions.ts` / `aiDailyRefinement.ts` | 规则引擎生成建议与细化内容 |
...
- **DailyRefinement**：date, markdown, checklist[], generatedAt（按日期存储细化结果）
...
- 6 个测试文件，覆盖表单校验、持久化、日历聚合、进度统计、AI 建议与细化
...
- 首页 `AiSuggestionsPanel` 基于计划数据生成：本周概览、负载警告（>5h/天）、进度提醒
...
- 详情页「细化当天计划」按钮，生成含学习材料、练习、完成标准的 Markdown

- 完成标准 checklist 可勾选，持久化至 localStorage
...
- 使用 Cursor Agent 完成需求拆解、分模块实现与测试编写

- MCP（HeroUI 文档）辅助 UI 组件选型参考
